...
| --- | --- | --- | --- | --- | --- |
| 1 | Solution 最小闭环 | 新增独立的 solution 内容闭环，不从旧 `plan` / `analyze-bug` 迁移，也不绑定 Git delivery 生命周期。 | 小 feature、小 fix、小 perf/test/docs/build 等需求能走 `solution -> solution-task -> solution-execute -> solution-review`；产物初始写入当前分支对应的 `.codex/timeline/<branch-type>/<branch-name>/`；fix 的复现、根因、修复方案被纳入 `SOLUTION.md`；perf 的基线、瓶颈和验证链路被纳入 `SOLUTION.md`；review 可记录通过、需修复或升级 MVP；solution 生成后能提示后续 `delivery-branch` rename checkpoint，但不直接改名。 | 待验收 | in-progress |
