...
- 旧 branch/worktree workflow 的 Hooks 仍使用 `WORKFLOW_STATE.json` 保护阶段边界；这不是 solution workflow 的主状态模型。
...
- 旧 branch/worktree workflow 阶段保护依赖 `plan/<type>/<branch-name>/WORKFLOW_STATE.json`。
...
| `solution` | 否 | 是 | 进入 pre-solution discussion，确认 type、目标和范围后写入 active slice 的 solution 与 state。 |
| `solution-task` | 否 | 是 | 从 active slice 的 solution 文件生成 task 文件，并推进到执行阶段。 |
| `solution-execute` | 否 | 是 | 执行 active slice 的 task，更新 task 和 state，完成后进入 review。 |
| `solution-review` | 否 | 是 | 审查 active slice 的实现和过程记录，写入 review 结论并决定是否回修。 |
...
Solution 内容闭环（小需求默认推荐）：

```text
$porter-codex-plugin:solution -> $porter-codex-plugin:solution-task -> $porter-codex-plugin:solution-execute -> $porter-codex-plugin:solution-review
```
...
Git commit / delivery 是 review 通过后的交付线，和 solution 内容闭环协同但不强绑定。当前 MVP 只明确这条边界，不声称已经实现完整 `delivery-*` Git 生命周期。
...
| `.codex/timeline/<timeline-name>/states/<slice-id>-<type>-<slug>.json` | 保存完整 workflow state、当前 skill、下一 skill 和 allowed outputs。 |

Branch/worktree workflow state：
