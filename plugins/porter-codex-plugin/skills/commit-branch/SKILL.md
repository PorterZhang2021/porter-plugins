---
name: commit-branch
description: Create branch workflow commits with conventional commit messages
...
- commit 完成后先询问用户是否还要调整提交内容或提交信息；如果没有，再提示用户显式调用 `$porter-codex-plugin:create-pr` 或 `$porter-codex-plugin:merge-branch-to-base`。

## Usage

Basic usage:
```
$porter-codex-plugin:commit-branch
```

With options:
```
$porter-codex-plugin:commit-branch --no-verify
$porter-codex-plugin:commit-branch --style=full
$porter-codex-plugin:commit-branch --style=full --type=feat
```
...
```
$porter-codex-plugin:commit-branch → $porter-codex-plugin:merge-branch-to-base  # Merge directly to main (solo projects)
$porter-codex-plugin:commit-branch → $porter-codex-plugin:create-pr      # Push branch and open a PR (team review)
```
