---
name: execute-branch
description: 基于 branch workflow 按对应节奏逐任务执行 TASK.md
allowed-tools:
  - Bash
  - Read
  - Write
...
- 完成任务后不得自动进入 `$porter-codex-plugin:review-branch`、`$porter-codex-plugin:commit-branch`、`$porter-codex-plugin:create-pr` 或 `$porter-codex-plugin:merge-branch-to-base`。
...
2. 查找 `plan/<type>/<branch-name>/TASK.md`：
   ...
   - **不存在，且类型为 `feat`/`fix`/`refactor`/`test`** → 终止，提示必须先运行 `$porter-codex-plugin:task-branch`
...
3. 检查 `plan/<type>/<branch-name>/WORKFLOW_STATE.json`：
   - **不存在** → 允许继续，但说明当前分支缺少 workflow state，建议后续由 `$porter-codex-plugin:analyze-bug` / `$porter-codex-plugin:task-branch` 自动生成
...
```json
{
  ...
  "current_skill": "$porter-codex-plugin:execute-branch",
  "next_skill": "$porter-codex-plugin:review-branch",
  "alternate_next_skill": "$porter-codex-plugin:commit-branch"
}
```

2. 展示执行摘要并停止，询问：**"执行阶段已完成。还有要补充、调整或继续执行的任务吗？如果没有，请显式调用 `$porter-codex-plugin:review-branch` 做提交前审查，或显式调用 `$porter-codex-plugin:commit-branch` 提交。"**
