---
name: execute-worktree
description: 基于 worktree workflow 按对应节奏逐任务执行 TASK.md
allowed-tools:
  - Bash
  - Read
  - Write
...
- 完成任务后不得自动进入 `$porter-codex-plugin:review-worktree`、`$porter-codex-plugin:commit-worktree`、`$porter-codex-plugin:create-pr` 或 `$porter-codex-plugin:merge-worktree-to-base`。
...
2. 查找 `plan/<type>/<branch-name>/TASK.md`：
   ...
   - **不存在，且类型为 `feat`/`fix`/`refactor`/`test`** → 终止，提示必须先运行 `$porter-codex-plugin:task-worktree`
...
3. 检查 `plan/<type>/<branch-name>/WORKFLOW_STATE.json`：
   - **不存在** → 允许继续，但说明当前分支缺少 workflow state，建议后续由 `$porter-codex-plugin:analyze-bug` / `$porter-codex-plugin:task-worktree` 自动生成
...
```json
{
  ...
  "current_skill": "$porter-codex-plugin:execute-worktree",
  "next_skill": "$porter-codex-plugin:review-worktree",
  "alternate_next_skill": "$porter-codex-plugin:commit-worktree"
}
```

2. 展示执行摘要并停止，询问：**"执行阶段已完成。还有要补充、调整或继续执行的任务吗？如果没有，请显式调用 `$porter-codex-plugin:review-worktree` 做提交前审查，或显式调用 `$porter-codex-plugin:commit-worktree` 提交。"**
