...
输出完成后，必须让用户确认"我的理解"是否正确。

优先使用运行环境支持的结构化提问能力：

```text
Codex 如支持 request_user_input 或类似结构化提问能力，可以用结构化问题确认用户意图；否则直接用普通对话确认。
如果运行环境不支持结构化提问，则直接在普通对话中列出选项并等待用户回复。
```
...
| --- | --- |
| `理解正确，执行任务` | 进入 Step 4。 |
| `不对，我补充一下` | 请用户用中文或英文说明哪里理解有误，收到补充后回到 Step 2 重新解析，合并原始输入和用户补充。 |
