---
name: plan-worktree
description: 基于当前分支类型，结对生成 worktree workflow 对应的规划文档
allowed-tools:
  - Bash
  - Read
  - Write
...
- 如果用户确认方案无补充，再提示用户显式调用 `$porter-codex-plugin:task-worktree` 进入下一阶段。
...
```text
Codex：PLAN.md 已生成。还有要补充或调整方案的吗？如果没有，请显式调用 $porter-codex-plugin:task-worktree 生成任务清单。
```

完整链路：

```text
plan-worktree -> task-worktree -> execute-worktree -> review-worktree? -> commit-worktree -> merge-worktree-to-base
```
