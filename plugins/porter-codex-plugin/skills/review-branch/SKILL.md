---
name: review-branch
description: branch workflow 提交前审查
---

# Review

在 `$porter-codex-plugin:execute-branch` 之后、`$porter-codex-plugin:commit-branch` 之前，对当前工作区中本次实现或修复的全部未提交改动进行审查。

`$porter-codex-plugin:review-branch` 是可选环节，不自动修改文件，不自动提交，也不强制阻断 `$porter-codex-plugin:commit-branch`。

## 阶段边界（强制）

- 本 skill 只做提交前审查，只输出 findings、open questions 和 summary。
- 不得自动修改文件，不得自动调用 `$porter-codex-plugin:execute-branch` 修复，也不得自动调用 `$porter-codex-plugin:commit-branch` 提交。
- 即使用户说"顺便修掉"或"没问题就提交"，也必须先完成审查并停止。
- 审查完成后先询问用户是否要补充审查范围或处理 findings；如果没有，再提示用户显式调用下一阶段 skill。

## 前置条件

1. **检查当前分支**：若在 `master` 分支上，立即终止并提示：`当前在 master 分支，请先运行 $porter-codex-plugin:new-branch 创建普通 Git 分支`
...
4. 读取工作区状态：
   ...
   git status --short
   ```
5. 若没有未提交改动，提示：`当前没有未提交改动。还有要补充审查的吗？如果没有，请显式调用 $porter-codex-plugin:commit-branch 或 $porter-codex-plugin:merge-branch-to-base`
...
2. 如果用户已显式调用 `$porter-codex-plugin:review-branch`，且当前环境支持 `multi_agent_v1.spawn_agent` 的 `code-reviewer` 子代理或等价的新上下文大模型，使用 `agent_type="code-reviewer"` 委托子代理审查 review brief 和 diff。
...
降级不是失败。不同 Codex 环境能力不同，但 `$porter-codex-plugin:review-branch` 的入口、目标和输出格式必须保持一致。
...
- <一句话总结是否建议先修复再 $porter-codex-plugin:commit-branch>
```
...
未发现阻断 $porter-codex-plugin:commit-branch 的问题。
```

## 收尾

- 如果存在 `P0` 或 `P1`，询问：**"发现提交前应处理的问题，是否进入修复？"**
- 如果没有 `P0` 或 `P1`，提示：**"Review 完成。还有要补充审查或处理的问题吗？如果没有，请显式调用 `$porter-codex-plugin:commit-branch` 提交。"**
