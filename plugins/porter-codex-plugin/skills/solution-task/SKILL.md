...
优先从 solution 文件的 Type Decision 读取 selected type。
...
如果 selected type 缺失、无法识别，或与 solution 内容明显冲突，停止并提示先修正 solution 文件。
...
- 所有任务必须使用 checkbox。
- 每个任务必须有 `验收标准` 和 `验证方式`。
- `验收标准` 必须能对应 solution 文件的 Acceptance，或明确说明这是结构审查、产物审查、基线采集等支撑性任务。
